...
<AccordionGroup>
  ...
  <Accordion title="New power-up types">
    Power-ups have two parts. First, add a case for your new type string inside `applyPowerUp(type)` — this is where the actual effect logic lives (speed, size, clone creation, etc.). Second, register the new color and type in `powerTypes[]` and add a `makePowerUp(color, type)` call inside `spawnLevel()` so the orb actually appears on the map.
  </Accordion>
