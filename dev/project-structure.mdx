...
- **Zoom controls** (`#zoomButton`, `#zoomToggleButton`) — the jump button and the ZOOMIES toggle
...
- **End screen** (`#endText`, `#endingMessage`, `#goBackButton`) — the ending overlay shown after fly-away, meteor, or final-boss cutscenes
...
- `#titleScreen` — full-screen title with difficulty buttons (Easy / Medium / Hard), the Play button, the animated Rudi preview canvas, and the controls reference card
...
- `#zoomToggleButton` / `#zoomButton` — ZOOMIES toggle and the jump/ascend button
...
- **`makeRoom(level)`** — clears `roomObjects[]`, sets the scene background and fog to the level theme's sky colour, recolours the ground and carpet, then builds level geometry based on the theme's `map` key (walls, islands, buildings, platforms, crystals, etc.). Calls `addBetterLevelDetails()` for extra decorative geometry per theme.
...
Three distinct ending sequences replace `updateGameplay` in the animation loop:

- **`triggerFlyAwayEnding()`** / **`updateFlyAwayEnding(dt)`** — Rudi floats upward and flies off screen. Triggered when the last level's treat goal is met in Easy or Medium mode.
...
function animate() {
  ...
  const dt = Math.min(clock.getDelta(), 0.033); // capped at ~30 fps minimum

  if (!gameStarted) {
    updateTitle(dt);   // spins the title-screen Rudi preview
    return;
  }

  if (finalBossEnding)   updateFinalBossEnding(dt);
  else if (flyAwayEnding) updateFlyAwayEnding(dt);
  else if (meteorEnding)  updateMeteorEnding(dt);
  else                    updateGameplay(dt);

  updateHunter(dt);   // moves the hunter dog in Medium/Hard mode
  updateDebris();     // advances explosion debris
  renderer.render(scene, camera);
}
```

`animate()` uses `THREE.Clock` for frame-delta and caps `dt` at `0.033` seconds to prevent physics tunnelling on slow frames. The title screen runs a separate `titleRenderer` that renders `titleScene` (containing only the spinning Rudi preview) into the `#titleRudi` div.
