---
title: "Endings: Fly-Away and Meteor Sequences Explained"
...
<Note>
  The "🐶 Oh no! Rudi got bored!" message fires regardless of whether the game was completed. The `completedGame` flag is `true` only when `currentLevel >= MAX_LEVELS` — if the fly-away fires on an earlier level through some edge case, the leaderboard prompt is skipped.
</Note>
